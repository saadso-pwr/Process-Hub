...
function BuilderInner() {
  ...
  const [prompt, setPrompt] = useState(SAMPLE_TEXT);
  ...
  // Populate the library on mount (client-only → no SSR hydration mismatch).
  useEffect(() => {
  ...
  const converge = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        diagram = coerceCanonical(json.diagram);
        setGenNote(json.source === "ai" ? "Generated with AI." : "Generated locally (no AI key set).");
      }
      if (!diagram) {
        diagram = parseProcessText(text);
        setGenNote("Generated locally.");
      }
      const built = canonicalToBuilder(diagram);
      const laidOut = layoutDiagram(built.nodes, built.edges);
      setNodes(laidOut);
      ...
      const built = canonicalToBuilder(diagram);
      setNodes(layoutDiagram(built.nodes, built.edges));
      setEdges(built.edges);
      setGenNote("Generated locally (offline).");
      setAiOpen(false);
      window.setTimeout(() => fitView({ padding: 0.15, duration: 400 }), 60);
    } finally {
      setGenerating(false);
    }
  }, [prompt, setEdges, setNodes, fitView]);
    ...
    <div style={{ display: "flex", height: "100%", fontFamily: FF }}>
      ...
      {aiOpen && (
        <AiModal
          ...
          onUseSample={() => setPrompt(SAMPLE_TEXT)}
        />
      )}
    </div>
  );
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            ...
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#6b6b66" }}>
              Describe the process in plain language. The result drops onto the canvas, fully
              editable.
            </p>
          </div>
          <button
            type="button"
            onClick={props.onClose}
            style={{ border: "none", background: "transparent", cursor: "pointer", fontSize: 22, color: "#999", lineHeight: 1 }}
            aria-label="Close"
          >
            ×
          </button>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <button
            ...
            style={{
              ...
              borderRadius: 10,
              border: "none",
              background: props.generating ? "#9aa0c8" : BRAND_BLUE,
              color: "#fff",
              cursor: props.generating ? "default" : "pointer",
            }}
          >
            {props.generating ? "Converging…" : "Converge"}
          </button>
          <button
            type="button"
